...
for($d = 0; $d < count($database_text); $d++){
	# <> with []
$database_text[$d] = str_replace("<","[",$database_text[$d]);
$database_text[$d] = str_replace(">","]",$database_text[$d]);

# "attack AT" with "attack"
$database_text[$d] = str_replace("attack AT","attack",$database_text[$d]);
# "ranged" with "shoot"
$database_text[$d] = str_replace("ranged","shoot",$database_text[$d]);
...
$white_test_size = imageftbbox($white_text_size, 0, $font_file, "Ag");
$white_dip = $white_test_size[1];
					#print "<br> dip: $dip";
...
$white_Y_positions = array(360- $white_dip, 360 - $white_dip);
...
#white box text positions
$black_Y_positions =array(387 + $black_text_size, 404 + $black_text_size, 421 +$black_text_size);
$black_X_positions = array(43, 43, 43);
$database_Y_positions =array( 387 + $black_text_size, 404 + $black_text_size, 421 +$black_text_size);
...
for( $i = 0; $i < count($white_text); $i++) {
	imagettftext($background_image, $white_text_size, 0, $white_X_positions[$i], $white_Y_positions[$i], $white,$font_file, $white_text[$i]);
}
#print name
...
for( $i = 0; $i < count($database_text); $i++) {
	$text_size_raw = imageftbbox($black_text_size,0, $font_file, $database_text[$i]);
...
$specail_box = imagettfbbox($black_text_size,0,$font_file, $specail_wrap);
