...
$database_text= array("1100","5", "4", "Straight +2");
$defense_stat = "500";
#assign the grey box text
$specail_text = "'Emergency Repairs' <declared>: Decrease this unit's HP by 100 and add 200 HP to 1 stowed Zoid. *Can be stacked.";
#-----------------------------------------------------------------------------------------------------------
...
#constant text
$text = array("Attack", "Shoot", "Range", "Area", "Defense", "Mobility", "Aptitude");
...
$base_offset = 3; # for boter problem between zoid and mobile base; Was 5 for AoE

# get ofset for middle of blue box -------------------
$defense_box = imagettfbbox($text_size,0,$font_file,"Defense");
$defense_half = ($defense_box[0]+$defense_box[2])/2;
$defense_mid = (415-320)/2 + 320;
print "<br> Midpoint of blue: $defense_mid";
print "<br> defense half: $defense_half";
$defense_start = $defense_mid - $defense_half;
$stat_box = imagettfbbox($text_size,0,$font_file,$defense_stat);
$stat_half = ($stat_box[0]+$stat_box[2])/2;
print "<br> stat half: $stat_half";
print "<br> Midpoint of blue: $defense_mid";
$stat_start = $defense_mid - $stat_half;
#------------------------------------------------


#text locations
# imagettftext position is lower left of string  [$yellow_mid + 6] for middle alisgnment
$Y_positions = array($yellow_mid+6, 420 + $text_size, 468 - 3, $yellow_mid+6, 420 + $text_size, $green_mid+6, $green_mid+6);
$X_positions = array(28+1, 105, 105, 228+$base_offset, $defense_start, 28+1,228+$base_offset);
...
$tmp = $im;
imagecopyresampled($im_overlay, $tmp, 308,470,295,466, 40, 40, 48, 48); #308 was 307 for AoE
#imagecopy($im_overlay, $tmp, 295,466,290,466, 48, 50);
...
$green = imagecolorallocate($image_sized, 21, 131, 68);
#color for lines
...
imagettftext($image_sized, $text_size, 0, 230, 84, $green ,$font_file, "Heal");

imagettftext($image_sized, $text_size, 0, $stat_start, 468 - 3, $black ,$font_file, $defense_stat);
...
$specail_text ="'Plasma Particle Cannons' [declared]: Reduce this Zoid's HP by 1000 and deal 1000 damage to up to 2 enemy Zoids in range, . 'name";
...
#initial split on ". '" for specail case
...
for( $i = 0; $i < $line_count; $i++) {
	...
	$text2= $lines[$i];
		...
		$text_size_wrap = imagettfbbox($Specail_text_size,0, $font_file, $text2);
					#print "<br> grey box width ";
		echo $X_limit-$X_position;
		while($text_size_wrap[4] > $X_limit-$X_position){
				#		print "<br> box size = $text_size_wrap[4]";
			$char_count--;
			$text2 = wordwrap($lines[$i], $char_count,"\n");
			$text_size_wrap = imagettfbbox($Specail_text_size,0, $font_file, $text2);
					#print "<br> new box size = $text_size_wrap[4]";
		}
		print "<br> Wrapped text: $text2";
		print "<br> char count: $char_count";
		$lines[$i] = $text2;
#		if($i+1 < $line_count){
