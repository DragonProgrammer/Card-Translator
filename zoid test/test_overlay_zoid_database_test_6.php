...
function imagettfstroketext(&$image, $size, $angle, $x, $y, &$textcolor, &$strokecolor, $font_file, $text, $px) {
    for($c1 = ($x-abs($px)); $c1 <= ($x+abs($px)); $c1++)
        for($c2 = ($y-abs($px)); $c2 <= ($y+abs($px)); $c2++)
            $bg = imagettftext($image, $size, $angle, $c1, $c2, $strokecolor, $font_file, $text);
   return imagettftext($image, $size, $angle, $x, $y, $textcolor, $font_file, $text);
}

function createLines(&$lines, $text_size, $X_limit, $X_position, &$Y_positions, $font_file){
#size the grey box text and split it into lines

#text wrap

 #               print "<br> doint text wrap";
for( $i = 0; $i < count($lines); $i++) {
    $text_size_raw = imagettfbbox($text_size,0, $font_file, $lines[$i]);
    $text2= $lines[$i];
        $char_count = 100;
        $text2 = wordwrap($lines[$i], $char_count,"\n");
        $text_size_wrap = imagettfbbox($text_size,0, $font_file, $text2);
...
$spacing_text = "Ag";
$spacing_box = $title = imagettfbbox($text_size,0, $font_file, $spacing_text);
$spacing = $spacing_box[1]+1;# - $Specail_text_size;

# create the Y positions of each line
$Y_position = 540 + $text_size;
$Y_positions = array($Y_position); 

 #               print "<br> about to make positions";
for($l = 1; $l < count($new_lines); $l++){
     $new_Y = $Y_positions[$l-1] + $text_size + $spacing;
    array_push($Y_positions, $new_Y);
}
    #print "<br> new lines";
    #for ($i = 0; $i<count($new_lines);$i++)
     #   print "<br> $new_lines[$i]";
...
$name_size = 25;
$name_font="ARIALN.TTF";

#this moves the name up so 'g' 'p' 'q' 'j' 'y' do not overlap
$spacing_text = "Ag";
$spacing_box = $title = imagettfbbox($name_size,0, $font_file, $spacing_text);
$spacing = $spacing_box[1];
$Name_pos[1] = $Name_pos[1]-$spacing+1;
#sive of Name area (based on 459 * 630) #128 to 321; 23 to 56
$name_width= 321 - 128;
$name_hieght = 56 - 23;

$name_box = imagettfbbox($name_size,0,$font_file,$Name);
while($name_box[2] > $name_width){
    $name_size--;
    $name_box = imagettfbbox($name_size,0,$font_file,$Name);
}
...
$font_file = 'times.TTF'; #does not seem to be pseed into createcard():
...
$Base_text = $lines;
if($specail_text != ""){
 #              print "<br> $specail_text";
do{
$Specail_text_size++;
#print "<br> text size is $Specail_text_size";
$Y_Positions = array("");  #reset y positions
$lines= $Base_text; #reset text
  #              print "<br> calling function";
createLines($lines, $Specail_text_size, $X_limit, $X_position, $Y_positions, $font_file);
$Y_end = $Y_positions[count($Y_positions)-1];
#    print "<br> in loop";
#    print "<br> Y end is $Y_end";
} while($Y_end < (593 + $spacing));

#----------------------------------
#TO DO see if i can make this check in the while check
if($Y_end > (593 + $spacing)){
    while($Y_end> (593 + $spacing)){
        $Specail_text_size--;
        $lines= $Base_text; #reset text
        $Y_Positions = array("");
...
if (count($lines) == 4 && $Specail_text_size > 10){
    print "<br> setting 4 line";
    $Specail_text_size = 10;
    $lines= $Base_text; #reset text
    $Y_Positions = array("");
  #              print "<br> calling function";
    createLines($lines, $Specail_text_size, $X_limit, $X_position, $Y_positions, $font_file);
...
if (count($lines) == 3 && $Specail_text_size > 14){
    print "<br> setting 3 line";
    $Specail_text_size = 14;
    $lines= $Base_text; #reset text
    $Y_Positions = array("");
  #              print "<br> calling function";
    createLines($lines, $Specail_text_size, $X_limit, $X_position, $Y_positions, $font_file);
...
#---------------------------------------------------------------------------------------
print "<br> font size = $Specail_text_size";

for($i=0; $i < count($lines); $i++){
   # print "<br> Line $i is: $lines[$i]";
   # print "<br> Line $i position is: $Y_positions[$i]";
...
for($l = 0; $l < count($lines); $l++){ 
	...
	else{ #               print "<br> Y positon for line $l is $Y_Positions[$l]";
        #print "<br> in else <br> $text";
